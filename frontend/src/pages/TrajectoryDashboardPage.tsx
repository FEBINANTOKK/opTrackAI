...
export function TrajectoryDashboardPage({ onLogout, onEditPreferences }: TrajectoryDashboardPageProps) {
  ...
  const highMatchCount = recommended.filter((opportunity) => opportunity.match >= 88).length
  const missingNodeCount = recommended.filter((opportunity) => !preferredSkills.includes('Node.js')).length
  ...
  const getReasons = (opportunity: Opportunity) => {
    ...
    return [
      skillMatch ? `Matches your ${skillMatch} skill` : `Builds on your ${topSkill} profile`,
      opportunity.mode === (preferences?.workMode ?? 'Remote')
        ? `Fits your ${opportunity.mode} preference`
        : `${opportunity.mode} option available`,
      opportunity.deadline === 'Rolling' ? 'Open on a rolling basis' : 'Deadline is coming up',
    ]
  }
          ...
          <div className="flex flex-col gap-4 px-6 py-6 sm:flex-row sm:items-center sm:justify-between lg:px-8">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm font-semibold text-slate-500">Your AI-curated opportunity feed.</p>
              </div>
            </div>
        ...
        <div className="grid gap-6 px-6 py-8 lg:grid-cols-[290px_1fr] lg:px-8">
          <aside className="space-y-4">
            <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              <p className="text-[11px] font-black uppercase tracking-[0.1em] text-[#1257d6]">Profile</p>
              <h2 className="mt-1 text-lg font-black tracking-[-0.02em] text-slate-900">Your preferences</h2>
              <dl className="mt-4 space-y-3">
                <ProfileRow label="Target" value={preferences?.target ?? 'Not set'} />
                <ProfileRow label="Opportunity" value={preferences?.opportunityType ?? 'Not set'} />
                <ProfileRow label="Location" value={preferences?.location || 'Not set'} />
                <ProfileRow label="JWT status" value={token ? 'Connected' : 'Waiting for backend'} />
              </dl>
            </div>

            <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              <p className="text-[11px] font-black uppercase tracking-[0.1em] text-[#1257d6]">Skills</p>
              <div className="mt-3 flex flex-wrap gap-2">
                {(preferences?.skills.length ? preferences.skills : ['React', 'TypeScript']).map((skill) => (
                  <span
          ...
          <div>
            <InsightBanner>
              You have {highMatchCount} high-match opportunities based on your {topSkill} skills and{' '}
              {preferences?.workMode ?? 'Remote'} preference.
            </InsightBanner>
            ...
            <div className="mt-4">
              <Card className="border-yellow-200 bg-yellow-50/80 p-4 shadow-sm hover:shadow-md">
                <div className="flex gap-3">
                  ...
                  <div>
                    <h2 className="text-sm font-black text-yellow-900">Skill gap insight</h2>
                    <p className="mt-1 text-sm font-bold leading-6 text-yellow-800">
                      You are missing Node.js for {missingNodeCount} high-match opportunities.
                    </p>
                  </div>
                </div>
              </Card>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-3">
              <StatCard icon="⭐" label="Top matches" value={`${recommended.length}`} />
              <StatCard icon="🌍" label="Preferred mode" value={preferences?.workMode ?? 'Remote'} />
              <StatCard icon="⏱" label="Time commitment" value={preferences?.timeCommitment ?? '1 week'} />
            </div>

            <div className="mt-6 grid gap-4">
              {recommended.map((opportunity) => (
                <Card className="group p-5 hover:-translate-y-1 hover:border-blue-200" key={opportunity.id}>
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <div className="min-w-48 space-y-3 lg:text-right">
                      <p className="text-sm font-bold text-slate-700">{opportunity.reward}</p>
                      <a
...
function ProfileRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div>
      <dt className="text-[11px] font-black uppercase tracking-[0.08em] text-slate-500">{label}</dt>
      <dd className={`mt-1 capitalize text-sm font-bold ${configured ? 'text-slate-800' : 'italic text-slate-400'}`}>
