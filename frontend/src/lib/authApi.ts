...
export async function fetchCurrentUser(
  token: string,
): Promise<{ user: User; preference: Preferences | null }> {
  const response = await fetch(`${API_BASE_URL}/users/me`, {
    ...
    throw new Error(
      "Unable to reach the backend while validating the login session.",
    );
  });

  const payload = await parseJson<{ user: User; preference?: Preferences }>(
    response,
  );
  return {
    user: payload.user,
